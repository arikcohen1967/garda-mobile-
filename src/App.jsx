...
export default function App() {
  ...
  const handleTriviaAnswer = (optIdx) => {
    ...
    const currentQ = TEEN_QUESTIONS[triviaIndex % TEEN_QUESTIONS.length];
  ...
  return (
    <div style={{ background: bgMain, minHeight: '100vh', color: textColor, fontFamily: 'system-ui, sans-serif', direction: 'rtl', paddingBottom: '40px', boxSizing: 'border-box' }}>
      ...
      {activeSosAlert && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: '50vh', background: 'rgba(239, 68, 68, 0.95)', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px', textAlign: 'center', color: '#fff', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', width: '100%', maxWidth: '320px' }}>
            <a href={`https://maps.google.com/?q=${activeSosAlert.lat},${activeSosAlert.lng}`} target="_blank" rel="noreferrer" style={{ flex: 1, padding: '10px', background: '#fff', color: '#ef4444', borderRadius: '12px', fontWeight: '900', textDecoration: 'none', fontSize: '13px', textAlign: 'center' }}>נווט 🗺️</a>
            <button onClick={dismissSos} style={{ flex: 1, padding: '10px', background: '#1e293b', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', fontSize: '13px' }}>בטל ✓</button>
          </div>
        </div>
      )}

      {/* Top Header */}
      <header style={{ background: cardBg, backdropFilter: 'blur(20px)', borderBottom: `1px solid ${borderColor}`, padding: '14px 16px', position: 'sticky', top: 0, zIndex: 1000, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <button onClick={() => setSidebarOpen(true)} style={{ background: isDark ? '#1e293b' : '#e2e8f0', color: textColor, border: 'none', width: '40px', height: '40px', borderRadius: '12px', fontSize: '20px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>☰</button>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <span style={{ fontSize: '11px', fontWeight: '800', color: isOnline ? '#22c55e' : '#f59e0b', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: isOnline ? '#22c55e' : '#f59e0b' }}></span>
        ...
        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          <div style={uniformHeaderBtnStyle(isDark, cardBg, textColor, borderColor)}><span>🌤️</span><span>{currentWeather.temp}</span></div>
          <a href={`https://www.waze.com/ul?q=${encodeURIComponent(HOTEL_ADDRESS)}&navigate=yes`} target="_blank" rel="noreferrer" style={{ ...uniformHeaderBtnStyle(isDark, cardBg, textColor, borderColor), textDecoration: 'none', background: '#33ccff', color: '#000', borderColor: '#33ccff' }}>{WAZE_SVG} למלון</a>
          <button onClick={triggerSos} style={{ ...uniformHeaderBtnStyle(isDark, cardBg, textColor, borderColor), background: '#ef4444', color: '#fff', borderColor: '#ef4444', border: 'none', cursor: 'pointer' }}>🚨 SOS</button>
        </div>
      </header>
      ...
      <aside style={{ position: 'fixed', top: 0, bottom: 0, right: 0, width: '280px', background: cardBg, zIndex: 2600, transform: sidebarOpen ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.3s ease', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: '10px', boxSizing: 'border-box', borderLeft: `1px solid ${borderColor}`, overflowY: 'auto' }}>
        ...
        <button onClick={() => setThemeMode(isDark ? 'light' : 'dark')} style={{ padding: '12px', borderRadius: '12px', background: isDark ? '#334155' : '#e2e8f0', color: textColor, border: 'none', fontWeight: '800', cursor: 'pointer', textAlign: 'center' }}>{isDark ? '☀️ מצב בהיר' : '🌙 מצב כהה'}</button>
      ...
      <main style={{ padding: '20px 16px', maxWidth: '600px', margin: '0 auto', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '10px', marginBottom: '20px', scrollbarWidth: 'none' }}>
          {INITIAL_TRIP_DAYS.map((d, i) => (
            <button key={i} onClick={() => setActiveDay(i)} style={{ flex: '1 0 auto', padding: '12px 16px', borderRadius: '16px', background: activeDay === i ? accentGradient : cardBg, color: activeDay === i ? '#fff' : textColor, border: `1px solid ${activeDay === i ? 'transparent' : borderColor}`, fontSize: '13px', fontWeight: '800', cursor: 'pointer', boxShadow: cardShadow }}>{d.label}</button>
          ))}
        </div>

        <div style={{ background: cardBg, border: `1px solid ${borderColor}`, borderRadius: '24px', padding: '24px', boxShadow: cardShadow }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: cardBg, color: textColor, padding: modalType === 'radar' ? '16px' : '24px', borderRadius: modalType === 'radar' ? 0 : '24px', width: modalType === 'radar' ? '100vw' : '100%', height: modalType === 'radar' ? '100vh' : 'auto', maxWidth: modalType === 'radar' ? 'none' : '450px', maxHeight: modalType === 'radar' ? 'none' : '85vh', overflowY: 'auto', border: modalType === 'radar' ? 'none' : `1px solid ${borderColor}`, boxShadow: cardShadow, boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
            ...
            {modalType === 'radar' && (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <button onClick={() => navigator.geolocation.getCurrentPosition(pos => broadcastMyLocation(pos.coords))} style={{ padding: '14px', background: accentGradient, color: '#fff', border: 'none', borderRadius: '14px', fontWeight: '800', cursor: 'pointer' }}>📍 עדכן מיקום GPS</button>
              </div>
            )}

            {modalType === 'timer' && (
              <div>
                <p style={{ fontSize: '13px', color: textSub, marginBottom: '14px' }}>בחר מצב זמן מהיר:</p>
            ...
            {modalType === 'trivia' && (
              <div>
                ...
                <p style={{ fontSize: '15px', fontWeight: '800', marginBottom: '14px' }}>{TEEN_QUESTIONS[triviaIndex % TEEN_QUESTIONS.length].q}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {TEEN_QUESTIONS[triviaIndex % TEEN_QUESTIONS.length].options.map((opt, oIdx) => (
            ...
            {modalType === 'emergency' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ background: isDark ? 'rgba(59, 130, 246, 0.1)' : '#eff6ff', border: '1px solid rgba(59, 130, 246, 0.3)', padding: '14px', borderRadius: '14px' }}>
                  <strong style={{ display: 'block', marginBottom: '4px', fontSize: '14px', color: '#3b82f6' }}>🇮🇱 שגרירות ישראל ברומא</strong>
                  <a href="tel:+3906361981" style={{ display: 'block', padding: '10px', background: '#3b82f6', color: '#fff', textAlign: 'center', borderRadius: '10px', fontWeight: '800', textDecoration: 'none', fontSize: '13px' }}>📞 חיוג לשגרירות</a>
                </div>
              </div>
            )}
            ...
            {modalType === 'parking' && (
              <div>
                ...
                <button onClick={() => { alert('החניה נשמרה!'); setModalType(null); }} style={{ width: '100%', padding: '12px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer' }}>שמור מיקום GPS</button>
              </div>
            )}
...
const uniformHeaderBtnStyle = (isDark, cardBg, textColor, borderColor) => ({
  height: '36px', padding: '0 8px', borderRadius: '10px', background: cardBg, color: textColor, border: `1px solid ${borderColor}`, fontWeight: '800', fontSize: '11px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', boxSizing: 'border-box', cursor: 'pointer', whiteSpace: 'nowrap'
});

const menuBtnStyle = {
  background: 'transparent', border: 'none', color: 'inherit', padding: '12px 14px', borderRadius: '12px', textAlign: 'right', fontWeight: '700', fontSize: '14px', cursor: 'pointer'
};

const timerPresetBtn = {
  padding: '12px', borderRadius: '12px', background: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6', border: '1.5px solid rgba(59, 130, 246, 0.3)', fontWeight: '800', fontSize: '14px', cursor: 'pointer'
};

const emergencyBtnStyle = {
  padding: '12px', borderRadius: '12px', background: '#fee2e2', color: '#ef4444', fontWeight: '800', fontSize: '12px', textAlign: 'center', textDecoration: 'none', border: '1.5px solid #fecaca', boxSizing: 'border-box'
};
